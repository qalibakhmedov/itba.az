...
-- ---- schema-phase1.sql: profiles + the three role-detail tables, test_attempts ----
...
drop policy if exists "admin read" on potential_ba_profiles;
create policy "admin read" on potential_ba_profiles for select
  using (is_admin());

drop policy if exists "admin read" on junior_ba_profiles;
create policy "admin read" on junior_ba_profiles for select
  using (is_admin());

drop policy if exists "admin read" on ba_professional_profiles;
